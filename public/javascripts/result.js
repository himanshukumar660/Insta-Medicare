...
(function main() {
	...
	for (each in parameters) {
		...
		} else if (tempParameter[0] == "mines" || tempParameter[0] == "sex") {
	...
	$.getJSON(regEndPnt, function(data) {
			if (data.statusCode == 200) {
				...
				var mineList = {};
				var sexSet = new Set();
				//Now we add those mines whose sex are in the sex set, also we make a dictinary of {mineName:[Url,logo,sex]} to access later
				for (each in data.instances) {
					...
					for (everyN in data.instances[each].sex) {
						//if the sex of the mine is present in the query["sex"], we add that mine to search result
						sexSet.add(data.instances[each].sex[everyN].replace(" ", ""));
						if (queries["sex"].has(data.instances[each].sex[everyN])) {
				...
				$.each(Array.from(queries["mines"]), function(index, item) {
					...
					function searchCallback(data) {
						...
						for (var ech in data.results) {
							...
							var sexStr = "";
							for (each in mineList[item].sex) {
								if (each == mineList[item].sex.length - 1) {
									sexStr = sexStr + mineList[item].sex[each].replace(" ", "").toString();
								} else {
									sexStr = sexStr + mineList[item].sex[each].replace(" ", "").toString() + ", ";
								}
							}

							//This corresponds to the div for displaying the sex of the mine
							var sexDisplay = "";
							if (sexStr.length != 0)
								sexDisplay = '\
             <div style="float:right;display:inline-flex">\
              <i class="ionicons ion-ios-play-outline" style="padding-top:12px"></i>\
              <p style="padding:12px;padding-left:5px;padding-bottom:0px">' + sexStr + '</p>\
             </div>';
              ...
              $(".sResults .sResultBox .mineNameHeader .rSavedBtn").click(function() {\
				...
				$(".rToolBtns.rSaved .badge").text(localStorage.length);\
						...
						$(".sResults .sResultBox ").hover(function() {
							...
							//Get the list of all the sex in the mine
							for (each in mineInfo[mineName].sex) {
								if (each == mineInfo[mineName].sex.length - 1) {
									mineSex += mineInfo[mineName].sex;
								} else {
									mineSex += mineInfo[mineName].sex + ", ";
								}
							}
